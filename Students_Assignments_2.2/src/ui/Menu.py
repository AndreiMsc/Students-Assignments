...
class Menu:
    ...
    def ui_run(self):
        ...
        while True:
            cmds_list = {"1":self.__ui_add, "2":self.__ui_remove, "3":self.__ui_update, "4":self.__ui_list, "5":self.__ui_statistics}
            com_list = ["1","2","3","4","5","6"]
            cmd = self.__ui_read_command("Type <1> to add/give, <2> to remove, <3> to update/evaluate, <4> to list, <5> to list statistics, <6> to exit..")
            if cmd in com_list:
                if cmd == "6":
