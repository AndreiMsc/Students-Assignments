...
class Menu:
    ...
    def __ui_give_assignment(self):
        ...
        com_list = ["1","2"]
        cmd = self.__ui_read_command("    Type <1> to give assignment to a student, <2> to add assignment <3> if you changed your mind..")
        if cmd in com_list:
            if cmd == "3":
                return
            else:
    ...
    def __ui_give_assignment_by_group(self):
        ...
        try:
            ...
            print(ve)
            return 0
        self.__assign_ctrl.find_assignment_by_id(assign_id)
        ...
        for stud in self.__stud_ctrl.list_all_students():
            st=str(stud)
            st = st.split(' ')
            if st[len(st)-1] == str(stud_group):
                self.__grade_ctrl.give_assignment(assign_id, int(st[0]))
                ok=True
        if ok==False:
